...
public class LyricUtils {
    ...
    public void readLyricFile(File file){
        if(file == null || !file.exists()){
            ...
            //2.排序
            Collections.sort(lyrics, new Comparator<Lyric>() {//这种方法时升序
                @Override
                public int compare(Lyric lhs, Lyric rhs) {
                    if(lhs.getShowTime() < rhs.getShowTime()){
                        return  -1;//按照return -1的排列
            ...
            for(int i=0;i<lyrics.size();i++){
                ...
                if(i+1 < lyrics.size()){
                    ...
                    oneLyric.setSleepTime(twoLyric.getShowTime()-oneLyric.getShowTime());
                }
            }
        }
